...
/// @title EightyColors
/// @notice The 80 colors of Dots. Matches the sorted gradient palette from
///         Jack Butcher's VV (EightyColors.sol) so the visual identity of the
///         merge ladder is preserved on MegaETH.
/// @dev    Gradient runs pink → red → orange → yellow → green → cyan → blue → purple.
library EightyColors {
    function COLORS() internal pure returns (string[80] memory) {
        return [
    ...
    /// @notice Index in `COLORS()` for the single-color mega-dot (divisor 7) terminal state.
    ///         VV renders this as black (#000) on white, not from the palette.
